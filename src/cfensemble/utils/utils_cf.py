...
def confidence2D(X, L, mode='brier', topk=(0, 0), scoring=None, greater_is_better=True, **kargs):
    ...
    **kargs
    ...
    conf_user, conf_item = kargs.get('conf_user', True), kargs.get('conf_item', False)  # # this has no effect on rank() method
    ...
    if len(p_threshold) > 0:
        ...
        # ... we then estimate how reliable the predict for that data point is by the ensemble
    ...
    ### return format 
    if kargs.get('outer_product', False) or (conf_user and conf_item): 
        # confidence level is computed as Wu * Wi
        assert mode != 'rank', "rank scores do not compute Wu and Wi separately!"
        ...
        if mode.startswith('rank'): 
            return W  # shouldn't get here
        else: 
            print('(confidence2D) Suppress Wi and only use Wu to compute Cui | mode: %s' % mode)
            W = np.outer(Wu, np.ones(n_items))
...
def confidence_pointwise_ensemble_prediction(R, labels, p_threshold, mode='item', suppress_majority=False, multiple=-1): 
    ...
    if suppress_majority: 
        if multiple < 0 or multiple is None: 
            ...
            multiple = ret['r_max_to_min']

    n_users = R.shape[0]
    Wi = np.zeros(R.shape[1])
    ...
    if mode.startswith(('l', 'u')) : # user-label correlation; i.e. compare classifier predictions to labels
        ...
        if masked_only: 
            ...
            for i in range(R.shape[0]):
                valid = np.where(R[:, i] != marker)
                r_valid = R[:, i][valid]
                W[i] = brier_score_loss(np.repeat(labels[i], len(r_valid)), r_valid)  # convert to confidence, the higher the better 
        else: 
            for i in range(R.shape[1]): 
                W[i] = brier_score_loss(np.repeat(labels[i], R.shape[0]),  R[:, i]) 
    else: 
        raise NotImplementedError("Mode: %s is not available yet." % mode)
...
def evalConfidenceMatrix(X, L=[], **kargs):
    ...
    **kargs
    ...
    params = {'ratio_users': kargs.get('ratio_users', 0.5), 
                ...
                'balance_class_weights': kargs.get('balance_class_weights', False if conf_measure == 'rank' else True), 
                'suppress_negative_examples': kargs.get('suppress_negative_examples', False),
                # 'mask_all_test': kargs.get('mask_all_test', True)

                'is_cascade': kargs.get('is_cascade', False),
                'policy_test': kargs.get('policy_test', policy),  # only relevant when 'is_cascade' is True
    ...
    print("... Balance class | balance sample size distribution? {t_sample}, balance class conf scores? {t_conf}".format(
        t_sample=params['balance_class'], t_conf=params['balance_class_weights']))
    ...
    ret = toConfidenceMatrix(X, L,           
                        ...
                        # message passing 
                        M=M,   # 2-tuple (R, L_train) or 3-tuple (R, L_train, Cr)
                        ...
                        # debug/testing
                        U=kargs.get('U', []),
                        L_true=kargs.get('L_true', []),  # test the accuracy of the unsupervised estimte of labeling 
                        fold=fold, 
...
def mask_over(X, L, C=None, U=None, ratio_users=0.5, 
    ...
    # [test] 
    # run_analysis()   
    # ... defer this to the main subroutine toConfidenceMatrix.balance_class_weights()
...
def balance_class_weights(C, X, L, p_threshold, Po=None, U=[], alpha=1.0, beta=1.0, gamma=0.5,
        suppress_max_class=False, is_cascade=True, discount_test=True,
    ...
    beta: the magnifiying factor for TP, FP and FN samples 
    ...
    if ret['n_min_class'] == 0: 
        ...
        print('(balance_class_weights) Warning: No minority class found in this batch => No-op! | n_max: {n_max}, n_min: {n_min} | dtype(L): {dtype} (expected int), value: {val}'.format(
    ...
    print('(balance_class_weights) Balancing class weights by considering size disparity ...')

    if is_test_split: 
        # some adjustments based on test split 
        
        # if unmask_on_test: 
        #     print('(balance_class_weights) Unmasking test data by assigning a minimum weight')
        #     unmask(C, L, U=U, test_cases=test_cases)
        pass

    ### A. re-weighting at global scale 

    # ... is polartiy matrix available? if so, it takes higher precedence over preference matrix (aka correctness matrix)
    ...
    cells_tp = (Mc > 0) & (Lh == min_class)
    cells_tn = (Mc > 0) & (Lh == max_class)
    ...
    Wfp = C[ cells_fp ]
    Wfn = C[ cells_fn ]
    msg += '... Before re-weighting  TP(+) | 5 numbers: {}\n'.format(common.five_number(Wtp))
    ...
    # standardize
    wmin = np.min([wtp, wtn, wfp, wfn])
    wtp, wtn, wfp, wfn = wtp/wmin, wtn/wmin, wfp/wmin, wfn/wmin

    msg += "... Reweight C inversely proprotional to samples sizes >\n"
    msg += "... wtp: {}, wtn: {}, wfp: {}, wfn: {} | wmin: {}".format(wtp, wtn, wfp, wfn, wmin)
    print(msg)
    ...
    #     div('(balance_class_weights) W(TP): {} <? W(TN): {} | multiple<tn/tp>: {} | w_min: {}'.format(Wtp, Wtn, multiple_eff, w_min))
    ...
    if beta > 1.0: 
        print('(balance_class_weights) amplifying weights associated with TPs, FPs, FNs {}'.format(beta))
    ...
    if suppress_max_class: 
        print('(balance_class_weights) suppress majority sample weights to {}'.format(w_min))
    ...
    if is_cascade and discount_test: 
        # gamma = 0.5
        print('(balance_class_weights) discount test sample weights by {}'.format(gamma))
        Cr, Ct = C[:,:n_train], C[:,n_train:]
        Ct = Ct * gamma
        C = np.hstack([Cr, Ct])
    ...
    msg = ''
    msg += '... After re-weighting  TP(+) | 5 numbers: {}\n'.format(common.five_number(Wtp))
    msg += '                        TN(-) | 5 numbers: {}\n'.format(common.five_number(Wtn))
    msg += '                        FP(-) | 5 numbers: {}\n'.format(common.five_number(Wfp))
    msg += '                        FN(+) | 5 numbers: {}\n'.format(common.five_number(Wfn))      
    print(msg)   
    ...
    ###################################################
    verify_confidence_matrix(C, X=X, L=L, Po=Mc, p_threshold=p_threshold, U=U, measure=conf_measure,
        message='(after) balanced + magnified (alpha={}, beta={}) | dtype: {}'.format(alpha, beta, 'test set' if is_estimated_labels else 'training set'), 
    ...
    # div("(balance_class_weights) Effective min-to-max weight ratio distribution: {ratios} | all > 1?".format(ratios=multiple_eff))
    # print('(balance_class_weights) weights of minority class multiplied by {m} on average >? standard multiple (non-weighted): {m0}'.format(m=np.mean(multiple_eff), m0=multiple))
    ...
    print('... class stats: {o}'.format(o=ret))
...
def verify_confidence_matrix(C, X, L, p_threshold, Po=None, U=[], measure='rank', message='', test_cases=[], plot=False, test_weight_constraints=True): 
    from analyzer import plot_confidence_matrix

    ranges = []
    if len(test_cases) == 0: test_cases = np.random.choice(range(C.shape[0]), 5)  
    print("(verify_confidence_matrix) Are the confidence scores assuming values as expected? ")
    if message: div(message, symbol='%', border=1)

    Mc, Lh = polarity_matrix(X, L, p_threshold) # X, pth -> Lh | L -> Mc
    if Po is not None: Mc = Po
    ...
    if test_weight_constraints: 
        ...
        # this depends on the filtering mechanism and whether L contains estimated labels
        n_correct = np.sum(C[Mc > 0])
        n_incorrect = np.sum(C[Mc < 0])
        message+=('(verify_confidence_matrix) n(TP+TN): {}, n(FP+FN): {}, ratio: {}'.format(n_correct, n_incorrect, n_correct/(n_correct+n_incorrect+0.0) ))

        # assert all(C[Mc != 0] > w_min), "(verify_confidence_matrix) All confidence scores corresponding to TP and TN should have a higher weight than the minimum: {}".format(np.random.choice(C[Mc], 10) )
        # ... this depends on the filtering mechanism
    ...
    wstats = {1: (np.min(Wpos), np.max(Wpos), np.mean(Wpos), np.median(Wpos)), 
              0: (np.min(Wneg), np.max(Wneg), np.mean(Wneg), np.median(Wneg)) }
    div('(verify_confidence_matrix) -- Weight distributions -- ', symbol='#')
    for cl in [1, 0]: 
        print('... Class {} | min: {}, max: {} | mean: {}, median: {}'.format(adict[cl], *wstats[cl]))
    print()
    ...
    for i in range(C.shape[0]): 
        if i in test_cases: 
            ...
            for cl in [1, 0]: 
                idx = np.where(L == cl)[0]
                idx_correct = np.where((L == cl) & (Mc[i] > 0))[0]   # C[i] is always >= 0.0 now

                scores = C[i][idx]
                scores_correct = C[i][idx_correct]

                m, M = np.min(scores), np.max(scores)
                if len(idx) > 0:
                    wx = np.random.choice(scores, 15)
                    msg = '... BP: [{}] | {} class | weight distribution:\n{}\n...... min: {}, max: {}, median: {}\n   ... ({})\n'.format(user_name, adict[cl], wx, m, M, np.median(scores), message)
                    if len(idx_correct) > 0: 
                        wx_correct = np.random.choice(scores_correct, 15)
                        m2, M2 = np.min(wx_correct), np.max(wx_correct)
                        msg += '...... weight_distribution (correct {} class):\n{}\n...... min: {}, max: {}, median: {}\n   ... ({})\n'.format(adict[cl], wx_correct, m2, M2, np.median(scores_correct), message)
                    print(msg)
    if measure == 'rank': 
        pass
...
def toConfidenceMatrix(X, L, **kargs):
    """
    Compute confidence matrix. First compute all confidence scores given the confidence measure (conf_measure, e.g. 'brier'), followed by 
    masking the unreliable entries (by zeroing them out). Mask function is determined by at least 4 factors (see Memo). 
    ...
    **kargs
    ...
    def mask_over0(W, R, labels, p_th=0.5, pos_label=1, neg_label=0, offset=-1):  
        # mask input W over the result of mask(R, labels)
        nU, nI = R.shape[0], R.shape[1]
        assert W.shape == R.shape

        missing_marker = kargs.get('missing_value', 0)
        R = maskFN(R, labels, p_threshold=p_th, fill=missing_marker)
        R = maskFP(R, labels,  p_threshold=p_th, fill=missing_marker)
        n_masked = np.sum(R == missing_marker)
        n_total = R.shape[0] * R.shape[1]
        print('... masked entries (nFP + nFN): {0} (out of {1} -> ratio: {2})'.format(n_masked, n_total, n_masked/(n_total+0.0)))

        # rows, cols = mask(R, labels, p_th=p_th, pos_label=pos_label, neg_label=neg_label)
        
        # rows, cols = np.where(R != missing_marker)
        # W_nonzero = W[rows, cols]
        # assert len(rows) == (n_total-n_masked), "n_nonzero: %d != %d" % (np.sum(R != missing_marker), n_total-n_masked)
        # print('... nTP+nTN: {0} =?= {1}'.format(len(rows), n_total-n_masked))
        # S = sparse.csr_matrix((W_nonzero, (rows, cols)), shape=(nU, nI))   

        # rows2, cols2 = np.where(R == missing_marker)
        # W_prime = W[rows2, cols2]
        # Sbar = sparse.csr_matrix((W_prime, (rows2, cols2)), shape=(nU, nI))   

        # return (S, Sbar)   

        W_prime = W.copy() 
        W_prime[R == missing_marker] = 0  # np.where(R == missing_marker) 

        if kargs.get('mask_all_test', True): 
            assert offset != -1
            n_users, n_test = R.shape[0], R.shape[1]-offset
            # maskT = np.zeros((n_users, n_test))
            W_prime = np.hstack( (W_prime[:, :offset], np.zeros((n_users, n_test))) )
            assert W.shape == W_prime.shape

        if kargs.get('sparse', True): 
            return sparse.csr_matrix(W_prime) # sparse.csr_matrix(W_prime) 
        return W_prime
    ...
    is_cascade = kargs.get('is_cascade', False)  
    policy_filtering = kargs.get('policy', 'user')
    policy_filtering_test = None if not is_cascade else kargs.get('policy_test', 'test')
    ...
    tSupervised = kargs.get('supervised', True) # 

    ### balance class weights in the confidence score 
    tBalanceClassWhileMasking = kargs.get('balance_class', False) 
    tBalanceClassWeights = kargs.get('balance_class_weights', False if conf_measure == 'rank' else True) 
    tSuppressMaxClass = kargs.get('suppress_negative_examples', False)
    tEstimatedLabels = kargs.get('estimated_labels', False)
    ...
    if len(p_threshold) == 0: 
        if tHasMessageFromTrainingSplit: 
            ...
            assert X_train is not None and L_train is not None
            ...
            policy_threshold = kargs.get('policy_threshold', 'prior')
            p_threshold = estimateProbThresholds(X_train, L=L_train, pos_label=pos_label, policy=policy_threshold) # policy: any policy that utilizes L
        else: 
    ...
    if len(L) == 0: 
        ...
        # if tHasMessageFromTrainingSplit: 
        #     # jointModel = estimateProbaByLabelMatrix(X_train, L_train, p_th=p_threshold, pos_label=1) # policy_threshold='prior' 
        #     L = lh = estimateLabels(X, L=[], p_th=p_threshold, pos_label=pos_label, M=(X_train, L_train) ) # joint_model=jointModel
    ...
    test_labels = L_ext = kargs.get('L_true', [])
    if tEstimatedLabels and len(L_ext) > 0: 
        accuracy = np.sum(lh == L_ext) / (len(L_ext)+0.0)
        div('(toConfidenceMatrix) accuracy of estimated labels: {} | n(L_ext): {}'.format(accuracy, len(L_ext)), symbol='#', border=2)
        # ... log: 
    ...
    C0 = confidence2D(X, L, mode=conf_measure, 
                ...
                outer_product=False, 
                    ...
                    conf_user=conf_user, conf_item=conf_item, 
    ...
    if tEstimatedLabels: # can only be true for a test split
        ...
        # use more conservative estimate of Mc
        if tConservative: 
                                ...
                                marker=null_marker, 
                                    supervised=tSupervised, 
                                        ...
                                        estimated_labels=tEstimatedLabels,  # is L an estimated label (test data) or true labels? 
                                            ...
                                            test_labels=L_ext,  # labels of the test set
    ...
    # ... initial polarity matrix Mc is determined

    ########################################################################
    # ... convert Mc: correctness matrix to polarity matrix (-1/negative, 0/neutral, 1/positive)
    # if isPolarityMatrix: 
    #     # no-op 
    #     pass 
    # else: 
    #     assert len(np.unique(Mc)) == 2, "Initial estimate of polarity via correctness matrix Mc | n(polarities): {}".format(len(np.unique(Mc)))
    #     Mc = to_polarity(Mc) # preference to polarity
    ...
    # C0: (default) confidence scores for all entries in X
    # Cui: masked confidence scores, where polarity neutral is 0
    # Po: polarity matrix (subsummed by Pc)
...
def to_mean_vector(X, L, **kargs):
    ...
    policy_filtering = kargs.get('policy', 'user')   # 'user', 'item', None (no filtering)
    null_marker = kargs.get('fill', 0) # [todo] marker for missing data
    topk_users, topk_items = 0, 0  # default, use all by setting to 0
    pos_label = kargs.get('pos_label', 1)
    n_users, n_items = X.shape[0], X.shape[1]
    ...
    ### balance class weights in the confidence score 
    tBalanceClassWeights = kargs.get('balance_class_weights', True)
...
def estimate_pref_threshold(Th, T, L=[], p_threshold=[], ratio_small_class=0.1, 
    ...
    correctness, _ = correctness_matrix(T, L, p_threshold)  # correctness matrix (Mc) entries: 1, if correct predictions (TP, TN); 0 o.w. 
...
def make_cn(C, Po, is_explicit=False, weight_neutral=0.0, sparsify=True):
    """
    Given polarity matrix (Po), mask the negative and neutral entries in the confidence matrix to be used for
    reconstructing the probability matrix. 
    """
    import scipy.sparse as sparse

    if is_explicit:  # for tExplicit to be True, isPreferenceScore must be False
        print("(make_cn) Using UNWEIGHTED Cw, non-weighted MF to approximate ratings ...")
        Cn = np.ones(C.shape) 
        if sparse.issparse(Po): 
            mask = Po.toarray()
            Cn[mask==0] = weight_neutral      # masking neutral
            Cn[mask < 0] = 0.0    # masking negative
            # Cn = sparse.csr_matrix(Cn)
        else: 
            Cn[Po==0] = weight_neutral  # masking neutral
            Cn[Po< 0] = 0.0  # masking negative
    else: 
        ...
        Cn[mask_negative] = 0.0  # masking negative
...
def make_cp(C, Po, is_explicit=False, sparsify=True):
    import scipy.sparse as sparse
...
def from_color_to_polarity(M, codes={}, verify=False): 
    """
        ...
        used for preference score approximation
    """
    import scipy.sparse as sparse
...
def test_polarity(T, labels, Pref=None, p_th=[], lh=[], name='T', pos_label=1, neg_label=0, title=''):
    ...
    # using true labels 
    Mc, Lh = correctness_matrix(T, labels, p_th)  # Mc is a (0, 1)-matrix

    # using estimated labels to estimate polarity matrix and labeling matrix
    Mct = Pref
    if Mct is None:
        Mct, Lht = correctness_matrix(T, lh, p_th)  # Mc is a (0, 1)-matrix
...
def estimate_polarity_simple(R, Lr, p_th, T, policy='median', labeling_model='simple',
    ...
    Mc, Lh = correctness_matrix(R, Lr, p_th)
...
def preference_matrix(**kargs):
    return correctness_matrix(**kargs) 
...
def t_confidence_measure(**kargs): 
            ...
            policy_threshold=policy_threshold, 
                ...
                supervised=True, # applicable to all policies (determines how p_threshold is computed if applicable)
                    conf_measure='brier', alpha=100, 
                        ...
                        balance_class_weights=tBalanceClassWeights,
    ...
    print('... delta(norm) on balance_class_weights: %f' % LA.norm(Cr.todense()-Cr2.todense(), 1))
