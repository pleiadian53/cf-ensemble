...
def reconstruct_by_preference(C, X, prefs, factors=[], labels=[], 
    ...
    if tHasTestLabels > 0:  
        Mc_ref, Lh_ref = uc.correctness_matrix(X, test_labels, p_threshold)  # note: pass X (not Xp)
    ...
    if binarize: 
        ...
        if pref_threshold < 0:   
            ...
            # >>> binarize
            Mc, Lh_X = uc.correctness_matrix(X, labels, p_threshold)  # note: pass X (not Xp), Mc: (approx) correctness matrix
            ...
            if verify: 
                ...
                Mc, Lh_X = uc.correctness_matrix(X, labels, p_threshold)  # note: pass X (not Xp)
    ...
    if verify: 
        if is_cascade: 
            ...
            ### test R?
            CMr, Lhr = uc.correctness_matrix(R, Lr, p_threshold)  # Lh(T, p_threshold)
            ...
            ### test T
            CMt, Lht = uc.correctness_matrix(T, Lt, p_threshold)  # find ratio of alignment wrt true labels (not the estimted labels lh) because we want to find how preference matrix is aligned wrt to true labels 
            ...
            if Po is not None: 
                ...
                Mct_max, Lht_max = correctness_matrix(T, lh, p_threshold)  # Mc is a (0, 1)-matrix
            ...
            ### overall X 
            CMx, Lhx = uc.correctness_matrix(X, L, p_threshold)  # find ratio of alignment wrt true labels (not the estimted labels lh) because we want to find how preference matrix is aligned wrt to true labels 
            ...
            CM, Lh = uc.correctness_matrix(X, L, p_threshold)  # Lh(T, p_threshold)
    ...
    if policy_opt.startswith('pref'):
        ...
        if replace_all: # params['predict_probs']: 
            ...
            if use_confidence_weights: 
                ...
                if W is None: 
                    W, _ = uc.correctness_matrix(X, L, p_threshold)
                else: 
        ...
        if is_cascade and len(L) > 0:
            ...
            Mct, Lht = uc.correctness_matrix(T, Lt, p_threshold)
...
def wmf_ensemble_iter(data, params, hyperparams={}, indices=[], vars=['wmfCV', 'wmfMetrics', ], kind='als', fold=-1, outer_fold=-1, null_marker=0, verbose=1, 
    ...
    if tPreferenceCalibration: 
        Mc, Lh_R = uc.correctness_matrix(R, L_train, p_threshold)  # CM entries: 1, if correct predictions (TP, TN); 0 o.w. 
...
def wmf_ensemble_preferred_ratings(data, params, hyperparams={}, indices=[], vars=['wmfCV', 'wmfMetrics', ], kind='als', fold=-1, outer_fold=-1, null_marker=0, verbose=1, 
    ...
    # compute reconstructed training data (so that later on we can test its utility for stacking)
    # Pr, Qr => Rh, use Rh in place of R whenever Cr == fill
    # CM = uc.correctness_matrix(R, L_train, p_threshold)
...
def wmf_ensemble_preferred_ratings2(data, params, hyperparams={}, indices=[], vars=['wmfCV', 'wmfMetrics', ], kind='als', fold=-1, outer_fold=-1, null_marker=0, verbose=1, 
    ...
    Cn = uc.make_cn(C0, Po, is_explicit=tExplicit, weight_neutral=weight_neutral)
    Cn = uc.balance_class_weights(Cn, X=X, L=L, Po=Po, p_threshold=p_threshold, U=U, 
                        alpha=params['alpha'], beta=params.get('beta', 1.0), 
    ...
    Cx = uc.make_cp(C0, Po, is_explicit=tExplicit)  # mask only neutrals (whose corresponding entries are dropped out of the cost function)
    Cx = uc.balance_class_weights(Cx, X=X, L=L, Po=Po, p_threshold=p_threshold, U=U, 
                        alpha=params['alpha'], beta=params.get('beta', 1.0), 
                            conf_measure=params['conf_measure'], 
                                suppress_max_class=params['suppress_negative_examples'], 
                                discount_test=params.get('discount_test', False), 
...
def wmf_ensemble_iter2(data, params, hyperparams={}, indices=[], vars=['wmfCV', 'wmfMetrics', ], kind='als', fold=-1, outer_fold=-1, null_marker=0, verbose=1, 
    ...
    McR, _ = uc.correctness_matrix(R, L_train, p_threshold)  # Mc: correct, Mh: estimated
    ...
    CX = uc.evalConfidenceMatrix(X, L=L, U=U,  
            ...
            # ... use L_true to pass L_test

            ratio_users=params['ratio_users'],  # this can be estimated from training split (R)
            ratio_small_class=params['ratio_small_class'], factor_small_class=params.get('factor_small_class', 1.0), 

            policy=params['policy'], # <<< filtering axis for training split in X
    ...
    ########################################################################################
    # ... determining Cn (n: neutral)
    # ... 1. masking negative (wrong predictions)
    # ... 2. masking neutral (undetermined)? Set beta_neutral to 0 to mask neutrals
    weight_neutral = 0.0

    Cn = uc.make_cn(C0, Po, is_explicit=tExplicit, weight_neutral=weight_neutral, sparsify=True)
    Cn = uc.balance_class_weights(Cn, X=X, L=L, Po=Po, p_threshold=p_threshold, U=U, 
                        alpha=params['alpha'], beta=params.get('beta', 1.0), 
                            conf_measure=params['conf_measure'], 
                                suppress_max_class=params['suppress_negative_examples'], 
    ...
    if tPreferenceCalibration: 
        ...
        MhX, Lhx = uc.correctness_matrix(X, lhx, p_threshold)  # Lh_X(X, p_threshould)
        ...
        McR, Lr = uc.correctness_matrix(R, lr, p_threshold)  # Mc: correct, Mh: estimated
        # Rh, pref_threshold, score = uc.calibrate_preference(Rh, Mc=McR, Lh=Lr, step=0.01, 
        #     policy=policy_calibration, message='train split (R)')

        MhT, Lht = uc.correctness_matrix(T, lht, p_threshold)  # Mh: estimated Mc
        ...
        # ... use L_test to get correctness matrix
        # Mc, _ = uc.correctness_matrix(X, np.hstack((L_train, L_test)), p_threshold)  # Lh(T, p_threshold)
        McT, Lt = uc.correctness_matrix(T, L_test, p_threshold)  # Lh(T, p_threshold)
